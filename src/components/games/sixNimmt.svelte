<script>
  ...
  async function submitScores(){
    let session = { date: new Date().toJSON().slice(0, 10).toString(), gameId: 3};
    ...
    for (const player of players) {
      if(player.selected){
        let playerSession = { game: "sixnimmtgame", gameSessionId: dataSession.gameSessionId.insertId, playerId: player.id, score: player.score};
  ...
  <div class="scores">
      <div class="sliders">
        {#each players as player}
          {#if player.selected}
            <ScoreTracker name={player.name} bind:score={player.score} colour={player.colour} minScore=-66 maxScore=66 titleVisible=true/>
          {/if}
        {/each}
