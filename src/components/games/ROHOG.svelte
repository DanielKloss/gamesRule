<script>
  ...
  async function submitScores(){
    let session = { date: new Date().toJSON().slice(0, 10).toString(), gameId: 4};
    ...
    for (const player of players) {
      if(player.selected){
        let playerSession = { game: "rohoggame", gameSessionId: dataSession.gameSessionId.insertId, playerId: player.id, score: player.score};
