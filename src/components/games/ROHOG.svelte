<script>
  ...
  async function submitScores(){
    ...
    const resultSession = await fetch(`/api/sessions`, {method: 'POST', body: JSON.stringify({date: today.getFullYear() + "-" + today.getMonth() + "-" + today.getDay(), gameId: 4}), headers: {'Content-Type': 'application/json'}});
    ...
    for (const player of players) {
      if(player.selected){
        const resultPlayerSession = await fetch(`/api/playerSessions`, {method: 'POST', body: JSON.stringify({game: "rohogGame", gameSessionId: dataSession.insertId, playerId: player.id, score: player.score}), headers: {'Content-Type': 'application/json'}});
